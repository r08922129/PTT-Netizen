...
class QABot(object):
    def __init__(self, corpus_path):
        with open(os.path.join(corpus_path, 'qa_pairs.pkl'), "rb") as f1,              open(os.path.join(corpus_path, 'corpus.pkl'), "rb") as f2,              open(os.path.join(corpus_path, 'dictionary.pkl'), "rb") as f3,              open(os.path.join(corpus_path, 'stopwords.txt')) as f4:

            self.qa_pairs = pickle.load(f1)
    ...
    def reply(self, query):
        ...
        answer = self.qa_pairs[sims.argmax()][1]
        return answer
